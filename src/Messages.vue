    ...
    <table v-if="messages.length > 0" class="table table-inbox table-hover">
        <tbody>
            <tr v-for="message in messages" :class="{ unread: typeof message.isRead !== 'undefined' && !message.isRead }">
                <td><input type="checkbox" v-model="message.isSelected"></td>
                <td>
                ...
                <td @click="openMessage(message)">{{ message.from.name }}</td>
                <td @click="openMessage(message)">{{ message.subject }}</td>
                <td @click="openMessage(message)"><i v-if="message.attachments.length > 0" class="fa fa-paperclip"></i></td>
                <td class="text-right" @click="openMessage(message)">{{ message.date.fromNow() }}</td>
            </tr>
        </tbody>
    </table>
